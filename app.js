...
// chama quando a crianca clica num botao de cor
...
// monta as telas do painel do professor
function desenharGraficosTCC() {
    ...
    if (totalZonas === 0) {
        ...
        for (let zona in bd.zonas) {
            ...
            let classeCss = "";
            if(zona === "Verde") classeCss = "zona-verde";
            if(zona === "Amarela") classeCss = "zona-amarela";
            if(zona === "Azul") classeCss = "zona-azul";
            if(zona === "Vermelha") classeCss = "zona-vermelha";
    ...
    for(let acao in bd.estrategias) {
        acoesArray.push({
            nome: acao,
            qtd: bd.estrategias[acao]
        });
    }
    
    // bota do maior pro menor
    acoesArray.sort(function(a, b) {
        return b.qtd - a.qtd;
    });
    ...
    if (acoesArray.length === 0) {
        ...
        for(let i=0; i < acoesArray.length; i++) {
            let obj = acoesArray[i];
